...
public class Matrix<V,F extends Field<V>> {
    ...
    public int hashCode() {
        ...
        code += rows; code *= 37;
        code += cols; code *= 37;
        code += rowMajor ? 1 : 0; code *= 37;
        for(int i = 0; i < flat.length; i++) {
            code += flat[i].hashCode(); code *= 37;
        }
        return code;
    }
    ...
    public Matrix<V,F> copy() {
        ...
        for(int i = 0; i < array.length; i++) { array[i] = flat[i]; }
    ...
    public V rowColumnInnerProduct(int row, Matrix<V,F> other, int col) {
        ...
        if(row < 0 || row >= rows) { throw new IllegalArgumentException(String.format("Row %d not in [%d, %d)", row, 0, rows)); }
        if(col < 0 || col >= other.cols) { throw new IllegalArgumentException(String.format("Col %d not in [%d, %d)", col, 0, other.cols)); }
        if(cols != other.rows) { throw new IllegalArgumentException("Inner row/column dimensions don't match"); }
        V sum = ops.zero();
